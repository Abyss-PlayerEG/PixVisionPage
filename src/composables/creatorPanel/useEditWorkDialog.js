...
import { useIndicatorSlider } from './useIndicatorSlider'
...
export const useEditWorkDialog = (deps) => {
  const { seriesList, loadSeries, handleUpdateWork, updateEditRadioIndicator } = deps
  ...
  // 单选指示器
  const radioIndicator = useIndicatorSlider()

  // 监听 isOriginal 变化，更新指示器
  watch(() => form.isOriginal, () => {
    if (radioIndicator.containerRef.value) {
  ...
  const open = (work) => {
    ...
    nextTick(() => {
      radioIndicator.update('.cp-radio-btn.active')
      if (updateEditRadioIndicator) updateEditRadioIndicator()
    })
  }
